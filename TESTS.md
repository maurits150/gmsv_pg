...
```sh
export GMSV_PG_ROOT=${GMSV_PG_ROOT:-/home/maurits/projects/gmsv_pg}
cd "$GMSV_PG_ROOT/pg"
premake4 gmake
cd project
make -B
...
```sh
export GARRYSMOD_DIR=${GARRYSMOD_DIR:-/home/maurits/.steam/steam/steamapps/common/GarrysMod/garrysmod}
cd "$GARRYSMOD_DIR"
./relaunch.sh
./wait_for_startup.sh
```
...
state.tests = {
	{
		...
		run = function(done)
			...
			if not assertEqual("objectCount type", type(pg.objectCount()), "number") then return end
			if not assertEqual("allocationCount type", type(pg.allocationCount()), "number") then return end
			if not assertEqual("deallocationCount type", type(pg.deallocationCount()), "number") then return end
			if not assertEqual("referenceCreatedCount type", type(pg.referenceCreatedCount()), "number") then return end
			if not assertEqual("referenceFreedCount type", type(pg.referenceFreedCount()), "number") then return end
	...
	},
	{
		...
		run = function(done)
			...
			assertThrows("invalid option key", function() pg.connect({ ["host port"] = "bad" }) end, "invalid")
			assertThrows("option key nul", function() pg.connect({ ["host\0bad"] = "127.0.0.1" }) end, "NUL")
			assertThrows("positional nul", function() pg.connect("127.0.0.1\0bad", "postgres", "postgres", "gmsv_pg_test", 55432) end, "NUL")
			assertThrows("option value nul", function() pg.connect({ host = "127.0.0.1\0bad" }) end, "NUL")
			...
			assertThrows("raw dsn timeout setter", function() dsn:setConnectTimeout(1) end, "connection string")
			pass("constructor forms")
			done()
		end
	},
	{
		name = "compatibility methods report supported and unsupported behavior",
		run = function(done)
			local db = pg.connect("127.0.0.1", "postgres", "postgres", "gmsv_pg_test", 55432)
			...
			assertThrows("OPTION_CACHE", function() query:setOption(pg.OPTION_CACHE, true) end, "OPTION_CACHE")
	...
	{
		name = "multi-statement raw query result chain",
		run = function(done)
			local query = state.db:query("SELECT 1::int4 AS value; SELECT 2::int4 AS value")
			function query:onSuccess(rows)
				if not assertEqual("multi statement first result", rows[1].value, 1) then return end
				if not assertEqual("multi statement has more", query:hasMoreResults(), true) then return end
				local nextRows = query:getNextResults()
				if not assertEqual("multi statement second result", nextRows[1].value, 2) then return end
				if not assertEqual("multi statement no more", query:hasMoreResults(), false) then return end
				pass("multi-statement result chain")
				done()
			end
			function query:onError(err)
				fail("multi statement", err)
			end
			query:start()
		end
	},
	{
		name = "multi-statement false enforces single raw statement",
		run = function(done)
			state.db:setMultiStatements(false)
			local query = state.db:query("SELECT 1::int4 AS value; SELECT 2::int4 AS value")
			function query:onSuccess()
				state.db:setMultiStatements(true)
				fail("multi statement false", "unexpected success")
			end
			function query:onError(err)
				state.db:setMultiStatements(true)
				if not assertTruthy("multi statement false error", string.find(err, "multiple commands", 1, true) or string.find(err, "prepared statement", 1, true)) then return end
				pass("multi-statement false")
				done()
			end
			query:start()
		end
	...
	{
		name = "multi-statement mode snapshotted for queued raw query",
		run = function(done)
			local blocker = state.db:query("SELECT pg_sleep(0.2)")
			function blocker:onError(err) fail("mode snapshot blocker", err) end
			blocker:start()
			...
			function query:onSuccess()
				state.db:setMultiStatements(true)
				fail("mode snapshot", "unexpected success")
			end
			function query:onError(err)
				state.db:setMultiStatements(true)
				if not assertTruthy("mode snapshot error", string.find(err, "multiple commands", 1, true) or string.find(err, "prepared statement", 1, true)) then return end
				pass("multi-statement mode snapshot")
				done()
			end
			query:start()
			state.db:setMultiStatements(true)
	...
	{
		name = "query options snapshotted for queued raw query",
		run = function(done)
			local blocker = state.db:query("SELECT pg_sleep(0.2)")
			function blocker:onError(err) fail("option snapshot blocker", err) end
			blocker:start()

			local query = state.db:query("SELECT 7::int4 AS value")
			query:setOption(pg.OPTION_NUMERIC_FIELDS, true)
			function query:onSuccess(rows)
				if not assertEqual("numeric option snapshot", rows[1][1], 7) then return end
				if not assertEqual("named absent after snapshot", rows[1].value, nil) then return end
				pass("query option snapshot")
				done()
			end
			function query:onError(err) fail("option snapshot", err) end
			query:start()
			query:setOption(pg.OPTION_NUMERIC_FIELDS, false)
		end
	},
	{
		...
		run = function(done)
			...
			function tx:onSuccess(results)
				if not assertEqual("transaction results raw name", results[1][1].name, "bob") then return end
				if not assertEqual("transaction results prepared name", results[2][1].name, "carol") then return end
	...
	{
		name = "transaction raw child enforces single statement",
		run = function(done)
			state.db:setMultiStatements(true)
			local tx = state.db:createTransaction()
			tx:addQuery(state.db:query("SELECT 1::int4 AS value; SELECT 2::int4 AS value"))
			function tx:onSuccess()
				fail("transaction raw child single", "unexpected success")
			end
			function tx:onError(err)
				if not assertTruthy("transaction raw child error", string.find(err, "multiple commands", 1, true) or string.find(err, "prepared statement", 1, true)) then return end
				pass("transaction raw child single statement")
				done()
			end
			tx:start()
		end
	...
	{
		name = "transaction rejects commented transaction control",
		run = function(done)
			runQuery(state.db, "SELECT count(*)::int AS count_before FROM gmsv_pg_suite", function(beforeRows)
				...
				local tx = state.db:createTransaction()
				tx:addQuery(state.db:query("INSERT INTO gmsv_pg_suite (name, active) VALUES ('tx-control', true)"))
				tx:addQuery(state.db:query("/* hidden */ COMMIT"))
				function tx:onSuccess()
					fail("transaction control", "unexpected success")
				end
				function tx:onError(err)
					if not assertTruthy("transaction control error", string.find(err, "transaction control", 1, true)) then return end
					runQuery(state.db, "SELECT count(*)::int AS count_after FROM gmsv_pg_suite", function(afterRows)
						if not assertEqual("transaction control rollback", afterRows[1].count_after, before) then return end
						pass("transaction control rejected")
						done()
					end)
				end
				tx:start()
	...
	{
		name = "transaction rejects cross-database child query",
		run = function(done)
			local other = pg.connect("127.0.0.1", "postgres", "postgres", "gmsv_pg_test", 55432)
			local tx = state.db:createTransaction()
			assertThrows("cross database child", function() tx:addQuery(other:query("SELECT 1")) end, "same database")
			pass("cross database child rejected")
			done()
		end
	},
	{
		name = "prepared query remains single statement in both raw modes",
		run = function(done)
			local function runPreparedCase(enabled, nextStep)
				state.db:setMultiStatements(enabled)
				local query = state.db:prepare("SELECT 1::int4 AS value; SELECT 2::int4 AS value")
				function query:onSuccess()
					state.db:setMultiStatements(true)
					fail("prepared multi " .. tostring(enabled), "unexpected success")
				end
				function query:onError(err)
					if not assertTruthy("prepared multi error " .. tostring(enabled), string.find(err, "multiple commands", 1, true) or string.find(err, "prepared statement", 1, true)) then return end
					nextStep()
				end
				query:start()
			end
			runPreparedCase(true, function()
				runPreparedCase(false, function()
					state.db:setMultiStatements(true)
					pass("prepared single statement")
					done()
				end)
			end)
		end
	...
	{
		name = "hasMoreResults false and getNextResults nil",
		run = function(done)
			runQuery(state.db, "SELECT 1::int4 AS value", function(rows, query)
				...
				if not assertEqual("getNextResults", query:getNextResults(), nil) then return end
				done()
			end)
		end
	}
...
state.tests = {
	...
	},
	{
		...
		run = function(done)
			local db = pg.connect("127.0.0.1", "postgres", "postgres", "gmsv_pg_test", 55432)
			local pre = pg.connect("127.0.0.1", "postgres", "postgres", "gmsv_pg_test", 55432)
			pre:disconnect(false)
			pre:connect()
			pre:wait()
			if not assertEqual("pre-disconnect then connect", pre:status(), pg.DATABASE_CONNECTED) then return end
			pre:disconnect(true)
	...
	},
	{
		...
		run = function(done)
			connectDatabase(function(db)
				assertThrows("escape nul", function() db:escape("a\0b") end, "NUL")
				assertThrows("charset nul", function() db:setCharacterSet("UTF8\0bad") end, "NUL")
	...
	},
	{
		...
		run = function(done)
			...
			assertThrows("ssl nul", function() db:setSSLSettings("key\0bad") end, "NUL")
	...
	{
		name = "commandStatus and oid returns numeric",
		run = function(done)
			connectDatabase(function(db)
				runQuery(db, "DROP TABLE IF EXISTS gmsv_pg_command_status", function()
					runQuery(db, "CREATE TABLE gmsv_pg_command_status (value int4); INSERT INTO gmsv_pg_command_status VALUES (1), (2); UPDATE gmsv_pg_command_status SET value = value + 1", function(rows, query)
					if not assertEqual("create commandStatus", query:commandStatus(), "CREATE TABLE") then return end
					if not assertEqual("create affectedRows", query:affectedRows(), 0) then return end
					local insertRows = query:getNextResults()
					if not assertEqual("insert commandStatus", query:commandStatus(), "INSERT 0 2") then return end
					if not assertEqual("insert affectedRows", query:affectedRows(), 2) then return end
					local updateRows = query:getNextResults()
					if not assertEqual("update commandStatus", query:commandStatus(), "UPDATE 2") then return end
					if not assertEqual("update affectedRows", query:affectedRows(), 2) then return end
					if not assertEqual("insert rows empty", #insertRows, 0) then return end
					if not assertEqual("update rows empty", #updateRows, 0) then return end
					runQuery(db, "SELECT 1::int4 AS value", function(rows, query)
					if not assertEqual("commandStatus", query:commandStatus(), "SELECT 1") then return end
	...
	},
	{
		...
		run = function(done)
			connectDatabase(function(db)
				...
				assertThrows("setString embedded nul", function() query:setString(1, "bad\0value") end, "NUL")
				local sparse = db:prepare("SELECT $1::text AS first, $2::text AS second")
				sparse:setString(2, "second")
				function sparse:onSuccess(rows)
					if not assertEqual("sparse first null", rows[1].first, nil) then return end
					if not assertEqual("sparse second", rows[1].second, "second") then return end
				end
				function sparse:onError(err) fail("sparse prepared", err) end
				sparse:start()
				sparse:wait()
	...
	{
		name = "putNewParameters is unsupported until parameter batches exist",
		run = function(done)
			connectDatabase(function(db)
				local query = db:prepare("SELECT $1::text AS value")
				query:setString(1, "first")
			...
			connectDatabase(function(db)
				connectDatabase(function(lockDb)
					runQuery(db, "TRUNCATE gmsv_pg_concurrency", function()
							runQuery(lockDb, "SELECT pg_advisory_lock(" .. lockKey .. ")", function()
							local blocker = db:query("SELECT pg_advisory_lock(" .. lockKey .. ")")
							local function finishAbortLoad()
							...
							blocker.onSuccess = finishAbortLoad
							blocker.onAborted = finishAbortLoad
			...
			connectDatabase(function(db)
				connectDatabase(function(lockDb)
					runQuery(db, "TRUNCATE gmsv_pg_stress", function()
						runQuery(lockDb, "SELECT pg_advisory_lock(" .. lockKey .. ")", function()
							local blocker = db:query("SELECT pg_advisory_lock(" .. lockKey .. ")")
							local function finishAbortStress()
							...
							blocker.onSuccess = finishAbortStress
							blocker.onAborted = finishAbortStress
...
- Multi-statement raw query result chains.
...
- `setMultiStatements(true)` default raw `db:query` result-chain behavior and `setMultiStatements(false)` single-statement enforcement.
- `setReadTimeout` / `setWriteTimeout` throwing honestly.
- `hasMoreResults()` / `getNextResults()` result-chain behavior.
- `commandStatus()` behavior.
...
- Additional command-status edge cases for future PostgreSQL commands.
