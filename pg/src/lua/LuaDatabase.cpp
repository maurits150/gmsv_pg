...
LUA_CLASS_FUNCTION(LuaDatabase, create) {
    std::shared_ptr<Database> createdDatabase;
    ...
    if (LUA->IsType(-1, GarrysMod::Lua::Type::Nil)) {
        LUA->Pop(3); //nil, pg, Global
        return 1;
    }
    LUA->Push(databaseTablePos);
    LUA->PushBool(true);
    LUA->RawSet(-3);
    LUA->Pop(3); //__weakDatabases, pg, Global

    return 1;
}

PG_LUA_FUNCTION(query) {
...
PG_LUA_FUNCTION(createTransaction) {
    auto database = LuaObject::getLuaObject<LuaDatabase>(LUA);
...
PG_LUA_FUNCTION(connect) {
    ...
    LUA->Pop(); // callback
    LUA->GetField(-1, "onReconnect");
    database->m_hasReconnectCallbacks = LUA->IsType(-1, GarrysMod::Lua::Type::Function);
    LUA->Pop(); // callback
    LUA->GetField(-1, "onReconnectFailed");
    database->m_hasReconnectCallbacks = database->m_hasReconnectCallbacks || LUA->IsType(-1, GarrysMod::Lua::Type::Function);
...
PG_LUA_FUNCTION(setSSLMode) {
    ...
    database->m_database->setSSLMode(static_cast<SSLMode>(mode));
    return 0;
}

PG_LUA_FUNCTION(setSSLSettings) {
    auto database = LuaObject::getLuaObject<LuaDatabase>(LUA);
...
void LuaDatabase::createMetaTable(ILuaBase *LUA) {
    LuaObject::TYPE_DATABASE = LUA->CreateMetaTable("PG Database");
...
void LuaDatabase::think(ILuaBase *LUA) {
    ...
    if (!reconnectEvents.empty() && this->m_tableReference != 0) {
        LUA->ReferencePush(this->m_tableReference);
        for (auto &event : reconnectEvents) {
            LUA->GetField(-1, event.first ? "onReconnect" : "onReconnectFailed");
            if (LUA->GetType(-1) == GarrysMod::Lua::Type::Function) {
                LUA->ReferencePush(this->m_tableReference);
                if (!event.first) {
                    LUA->PushString(event.second.c_str());
                    pcallWithErrorReporter(LUA, 2);
                } else {
                    pcallWithErrorReporter(LUA, 1);
                }
...
/** Creates a weak table for all currently used databases.
  ...
  *
  * Expects the pg table to be at the top of the stack.
  *
